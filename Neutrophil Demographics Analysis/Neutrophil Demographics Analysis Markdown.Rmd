---
title: "Neutrophil Demographics Analysis"
...
```{r}
# Import data
seahorse_data <- data.frame(read_excel("Input Data/2021_12_14 Seahorse Demographics Analysis.xlsx"))
...
```{r}
# Import data
phagocytosis_data <- data.frame(read_excel("Input Data/2021_12_14 Phagocytosis Demographics Analysis.xlsx"))
...
### Make demographics tables for the two datasets
...
### Assess normality of phagocytosis and seahorse data
...
### Statistical analysis for significant differences in function associated with demographic variables

To assess sex differences, a t-test was used for normally distributed data, and a Wilcox test was used for non-normally distributed data. For correlation testing with age and BMI, Spearman's correlation was used because it can detect both linear and monotonic correlations, whereas Pearson's correlation only detects linear correlations. 
...
write.table(seahorse_sex, file = "Output Tables/SeahorseSexTtest.csv", sep = "\t", row.names = FALSE)
```
```{r}
# Age correlation
...
write.table(seahorse_spearman_age, file = "Output Tables/SeahorseAgeSpearman.csv", sep = "\t", row.names = FALSE)
...
write.table(seahorse_spearman_bmi, file = "Output Tables/SeahorseBMISpearman.csv", sep = "\t", row.names = FALSE)
```
...
### Plot correlations between phagocytosis data and Seahorse metrics
...
### Plot specific correlations between phagocytosis and Seahorse parameters by sex

```{r}
# Phagocytosis v. OCR time to max
theme_set(theme_bw())
...
```
```{r}
# Phagocytosis v. ECAR time to max
theme_set(theme_bw())
